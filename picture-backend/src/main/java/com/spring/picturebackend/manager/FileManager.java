...
public class FileManager {
    ...
    public UploadPictureResult uploadPictureByUrl(String fileUrl, String uploadPathPrefix) {
        ...
        // 图片上传地址 构建一个文件名（uploadPath = uploadPathPrefix + uploadFileName）data_uuid.originFileName
        String uuid = RandomUtil.randomString(16);
        String originFileName = FileUtil.getName(fileUrl);
        String uploadFileName = String.format("%s_%s.%s", DateUtil.formatDate(new Date()), uuid, originFileName);
        String uploadPath = String.format("/%s/%s", uploadPathPrefix, uploadFileName);
        File file = null;
        try {
            // 创建临时文件（createTemPlate）将 url 下载的图片放到这个模板 file 中
            file = File.createTempFile(uploadPath, null); // 现在这个模板文件还是空的
